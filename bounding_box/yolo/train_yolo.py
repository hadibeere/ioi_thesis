...
def train(loader, model, optimizer, criterion, device):
    ...
    det_strides = [32, 16, 8]
    metrics = StatCollectorTrain(416, anchors, det_strides=det_strides, conf_threshold=0.5, nms_threshold=0.1,
...
if __name__ == '__main__':
    ...
    num_input = int(net_options['channels'])
    momentum = float(net_options['momentum'])
    decay = float(net_options['decay'])  # Penalty for regularisation
    learning_rate = float(net_options['learning_rate'])  # Initial learning rate

    train_transform = pr.TrainAugmentation(image_size, p=0.5)
    test_transform = pr.TestTransform(image_size)
    target_transform = pr.MatchAnchors(net_options['anchors'], net_options['num_anchors'], inp_dim=image_size,
    ...
    criterion = ls.YOLOLoss(delta_coord=float(net_options['delta_coord']), delta_obj=float(net_options['delta_obj']),
                            delta_noobj=float(net_options['delta_noobj']), delta_cls=float(net_options['delta_cls']))
    if args.use_focal_loss:
        criterion = fl.FocalLoss(gamma=float(net_options['gamma']), alpha=float(net_options['alpha']))
    ...
    for epoch in range(args.epochs):
        ...
        #)
        loss, precision, recall = train(train_loader, model, optimizer, criterion, device)
